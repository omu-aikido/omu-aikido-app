...
export function MyRanking({ props }: { props: MyRankingProps | undefined | null }) {
  ...
  return (
    <div
      className="my-4 p-2 sm:p-3 border border-slate-200 dark:border-slate-700 rounded-lg shadow bg-white dark:bg-slate-800"
      data-testid="my-ranking-container"
    >
      <h3 className="font-bold flex justify-center mb-2" data-testid="my-ranking-title">
        月間ストリーク
      </h3>
      <div
        className="flex flex-row gap-2 sm:gap-3 items-stretch"
      ...
      >
        ...
        >
          <span
            className="text-slate-500 dark:text-slate-400  sm: mb-0.5"
            data-testid="my-ranking-activity-label"
          >
            稽古回数
          </span>
          <span
            className="text-base sm:text-lg font-bold text-blue-700 dark:text-blue-200"
